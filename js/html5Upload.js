...
function html5Upload() {
    ...
    var settingOBj = {
        ...
        dropdown: function (e) {}, //DnD event setting
        ...
        abortReader: function (e, file) {}, //reader event setting
        startXHR: function (e, file) {},
        progressXHR: function (e, file) {},
        endXHR: function (e, file) {},
        loadXHR: function (e, file) {},
        errorXHR: function (e, file) {} //XHR event setting
    };
    ...
    var initial = function (options) {
        ...
        $(window).bind("drop", function (e) {
            if (e.target.id != 'imr_c_image_drag_area')
                uploadImgFocus = e.target.id || e.target.offsetParent.id || e.target.offsetParent.offsetParent.id;
            e.preventDefault();
        });
        //drag and drop style
        ...
        $(settings.uploadButton).bind("change", function (e) {
            getFiles(e.target.files);
            if ($(this).data('replace')) $(this).replaceWith($(this).clone(true));
        });
        $(settings.uploadArea).on("drop", function (e) {
            getFiles(e.originalEvent.dataTransfer.files)
        });
        //abort the upload
        $(settings.uploadAbort).bind("click", function () {
            queue = [''];
            reader.abort();
    ...
    var getFiles = function (filelist) {
        ...
        var list = {};
        for (var i in filelist) list[i] = filelist[i];
            ...
            if (filelist.length != totalNumber + filelist.length - settings.numLimitTotal) {
                for (var i = 0; i < filelist.length - (settings.numLimitTotal - totalNumber); i++) {
                    for (var key in filelist)
                        if (key == filelist.length - 1 - i) delete list[key];
                }
                list.length = settings.numLimitTotal - totalNumber;
            } else {
                console.error('Out of Total number');
                return;
            }
        }
        ...
        numSet = 0;
        sizeSet = 0;
        for (var i = 0; eval(numStatement); i++) {
        ...
        //upload size filter
        if (settings.sizeLimitSet)
            if (sizeSet > settings.sizeLimitSet) {
                console.error('Out of Set size');
                queue = [];
                return;
            }
        sizeTotal += sizeSet;
        if (settings.sizeLimitTotal)
            if (sizeTotal > settings.sizeLimitTotal) {
                console.error('Out of Total size');
                sizeTotal -= sizeSet;
                queue = [];
                return;
            }
    ...
    var uploadQueue = function () {
        if (queue.length) {
            ...
            if (!!~settings.mimeTypes.indexOf(file.type)) fileUpload(file);
                ...
                /*console.error('Wrong or Empty file type');/**/
                totalNumber--;
                fileUpload(file);
            }
        }
    ...
    var fileUpload = function (file) {
        ...
        reader.onloadstart = function (e) {
            settings.startReader(e, file)
        };
        reader.onprogress = function (e) {
            settings.progressReader(e, file)
        };
        reader.onload = function (e) {
            settings.loadReader(e, file);
            counter++;
        };
        reader.onloadend = function (e) {
            settings.endReader(e, file);
            uploadQueue();
        };
        reader.onabort = function (e) {
            settings.abortReader(e, file)
        };
    ...
    var ajaxTxing = function (ajaxData, file) {
        dataObject = {};
        dataObject[settings.ajaxData] = ajaxData;
        $.ajax({
            ...
            xhr: function () {
                ...
                xhr.upload.onloadstart = function (e) {
                    settings.startXHR(e, file)
                };
                xhr.upload.onprogress = function (e) {
                    settings.progressXHR(e, file)
                };
                xhr.upload.onload = function (e) {
                    settings.loadXHR(e, file)
                };
                xhr.upload.onloadend = function (e) {
                    settings.endXHR(e, file)
                };
                xhr.upload.onerror = function (e) {
                    settings.errorXHR(e, file);
                    uploadQueue();
                };
                return xhr;
            }
    ...
    return {
        paras: function (parameter) {
            return eval(parameter)
        },

        setParas: function (obj) {
            object = jQuery.extend(true, object, obj)
        },
        ...
        ajaxTxing: function (ajaxData, file) {
            ajaxTxing(ajaxData, file)
        }
    }
}
