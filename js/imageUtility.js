...
var imageUtility = (function () {
    var imgObj = [],
        imgReSize = [], // image size arrays
        pQueue = [],
        pCnt = 0,
        pLock = true, // file queues
    ...
    var normalQuality = function (image, scale, object) {
        var resultCV = document.createElement("canvas");
        resultCV.width = scale[1] || 55;
        resultCV.height = scale[2] || 55;
    ...
    var fineQuality = function (image, scale, object) {
        ...
        var sq = scale * scale, //pixel of source within target
            sw = cv.width,
            sh = cv.height, //source width height
            sx = 0,
            sy = 0,
            sIdx = 0, //source x y and pixel index of source array
            tw = Math.floor(sw * scale),
            th = Math.floor(sh * scale), //target width height
            tx = 0,
            ty = 0,
            tIdx = 0, //target x y and pixel index of target array
            TX = 0,
            TY = 0,
            yIdx = 0, //rounded tx ty
            w = 0,
            nw = 0,
            wx = 0,
            nwx = 0,
            ...
            sB = 0, //source RGB
            crossX = false,
            crossY = false, //scaled px cross current px border
        ...
        for (sy = 0; sy < sh; sy++) {
            ...
            for (sx = 0; sx < sw; sx++, sIdx += 4) {
                ...
                sR = sBuffer[sIdx + 0];
                sG = sBuffer[sIdx + 1];
                sB = sBuffer[sIdx + 2]; //get source RGB
                if (!crossX && !crossY) { //NO pixel cross
                    tBuffer[tIdx + 0] += sR * sq;
                    tBuffer[tIdx + 1] += sG * sq;
                    tBuffer[tIdx + 2] += sB * sq;
                    ...
                    tBuffer[tIdx + 3] += sR * nw;
                    tBuffer[tIdx + 4] += sG * nw;
                    tBuffer[tIdx + 5] += sB * nw;
                    ...
                    tBuffer[tIdx + 3 * tw + 0] += sR * nw;
                    tBuffer[tIdx + 3 * tw + 1] += sG * nw;
                    tBuffer[tIdx + 3 * tw + 2] += sB * nw;
                } else { //XY cross both
                    ...
                    tBuffer[tIdx + 3 * tw + 3] += sR * nw;
                    tBuffer[tIdx + 3 * tw + 4] += sG * nw;
                    tBuffer[tIdx + 3 * tw + 5] += sB * nw;
                }
            }
        }
    ...
    var imgObjPackage = function (idx, strData, file, imgSize, imgExif) {
        imgObj[idx] = [];
        imgObj[idx][0] = strData;
        imgObj[idx][1] = file.type;
        imgObj[idx][2] = file.name;
        ...
        imgObj[idx][5] = imgExif[0] || null;
        imgObj[idx][6] = imgExif[1] || null;
    ...
    var finishUpload = function (image, file, thumbnail, imgSize, imgExif, object) {
        ...
        if (object.canvasRotate)
            for (var i in imgObj) imgObj[i][7] = 1;
    ...
    return {
        ...
        cutSquare: function (canvas, width, height) {
            ...
            length -= 2;
            cutPosX = 0 | ((width - length) / 2);
            cutPosY = 0 | ((height - length) / 2);
            var newCanvas = document.createElement("canvas");
            newCanvas.width = length;
            newCanvas.height = length;
        ...
        thumbnail: function (image, object) {
            var thumWidth = object.width || 100,
                thumHeight = object.height || 100;
            var thuW = null,
                thuH = null;
        ...
        getSize: function (_FILE_) {
            var _A_ = [_FILE_],
                _B_ = 0;
            for (var _C_ = 0; _C_ < _A_.length; _C_++) {
                switch (typeof _A_[_C_]) {
                    ...
                    case "boolean":
                        ...
                        break;
                    case "string":
                        _B_ += 2 * _A_[_C_].length;
                        break;
                    case "object":
                        if (Object.prototype.toString.call(_A_[_C_]) != "[object Array]")
                            for (var _D_ in _A_[_C_]) _B_ += 2 * _D_.length;
                        for (var _D_ in _A_[_C_]) {
                            var _E_ = false;
                            for (var _F_ = 0; _F_ < _A_.length; _F_++) {
                                if (_A_[_F_] === _A_[_C_][_D_]) {
                                    _E_ = true;
                                    break;
                                }
                            }
        ...
        imageProcessor: function (image, file, thumbnail, imgSize, imgExif, object, sBuffer) {
            var html5 = new html5Upload(),
                idx = Object.keys(imgSize).length - imgReSize.length;
            if (imgReSize.length > +!!object.newImg) {
                if (imgReSize[0].Q == 'fine' && typeof window.Worker == "function") {
                    ...
                    var sw = cv.width,
                        sh = cv.height,
                        tw = Math.floor(sw * imgReSize[0].S),
                        th = Math.floor(sh * imgReSize[0].S),
                        sBuffer = sBuffer || cv.getContext("2d").getImageData(0, 0, sw, sh);
                    var resultCV = document.createElement("canvas");
                    resultCV.width = tw;
                    resultCV.height = th;
                    var resultCTX = resultCV.getContext("2d"),
                        resultIMG = resultCTX.getImageData(0, 0, tw, th),
                        tByteBuffer = resultIMG.data;
                    ...
                    imageWorker.onmessage = function (e) {
                        if (typeof (tByteBuffer.set) == "function") tByteBuffer.set(e.data);
                            ...
                            for (var i in e.data) tByteBuffer[i] = e.data[i];
                        resultCTX.putImageData(resultIMG, 0, 0);
                        ...
                        e.data = imageWorker = strData = resultIMG = null;
                        if (typeof (CollectGarbage) == "function") CollectGarbage();
                        imgReSize.shift();
                        imageUtility.imageProcessor(image, file, thumbnail, imgSize, imgExif, object, sBuffer);
                    }
                    imageWorker.onerror = function (e) {
                        console.error(e.message + ' in ' + e.filename + ' Line: ' + e.lineno)
                    }
                } else {
        ...
        readProcess: function (e, file, object, imgExif) {
            var html5 = new html5Upload();
            object = object || {};
            imgExif = imgExif.length ? imgExif : [null, null, 1];
            if (!!~html5.paras('imageMime').indexOf(file.type)) {
                ...
                function processInterval() {
                    if (pLock) {
                        ...
                        var imgQualityArr = object.imgQuality || [];
                        object.imgQuality = [];
